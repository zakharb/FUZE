...
function Table() {
  ...
  // use effects
  useEffect(() => {
    fetchMessagesData(setData, setRawData)
  ...
  const handleEditClick = async (row) => {
    try {
      const editData = await fetchMessagesRuleGet(row.original.id);
      setFormData({
        name: editData.name,
  ...
  const handleApplyFilters = () => {
    fetchMessagesFilter(filters, startDate, endDate, setData, setRawData);
  };
...
function EditForm({ formData, setFormData, row, data, onClose }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const editData = await fetchMessagesRuleEdit(row.original.id, formData);
    } catch (error) {
      console.error(error);
    }
    onClose();
...
const handlePlaybookClick = async (event) => {
  ...
  try {
    const response = await fetchMessagesPlaybook(row.original.id, controller);
