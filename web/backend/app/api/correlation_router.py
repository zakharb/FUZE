...
from app.api.correlation_model import RuleModel, UpdateRuleModel
...
async def create_rule(request: Request, 
    ...
    result = await request.app.mongodb["correlation"].insert_one(rule)
...
async def list_rules(request: Request):
    list_rules = []
    for doc in await request.app.mongodb["correlation"].find().to_list(length=1000):
       doc['id'] = str(doc['_id'])
       del doc['_id']
...
async def get_rule(id: str, 
                    request: Request):
    if (rule := await request.app.mongodb["correlation"].find_one({"_id": ObjectId(id) })) is not None:
...
async def update_rule(id: str, 
    ...
    col = request.app.mongodb["correlation"]
...
async def delete_rule(id: str, 
                      request: Request):
    delete_result = await request.app.mongodb["correlation"].delete_one(
        {"_id": ObjectId(id)})
...
async def copy_rule(id: str, request: Request):
    ...
    rule = await request.app.mongodb["correlation"].find_one({"_id": ObjectId(id)})
    if not rule:
        raise HTTPException(status_code=404, detail=f"Rule {id} not found")
    ...
    result = await request.app.mongodb["correlation"].insert_one(copy)
...
async def export_rules(request: Request):
    list_rules = []
    for doc in await request.app.mongodb["correlation"].find().to_list(length=10000):
       del doc['_id']
       list_rules.append(doc)
    return list_rules
...
async def import_rules(request: Request):
    request.app.mongodb["correlation"].drop()
...
async def import_rules(request: Request, file: UploadFile = File(...)):
    ...
    correlation = json.loads(contents)
    rules = []
    for raw_rule in correlation:
        rules.append({
            'name': raw_rule['name'],
            'src': raw_rule['src'],
            'dst': raw_rule['dst'],
