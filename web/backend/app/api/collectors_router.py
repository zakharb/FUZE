...
from app.api.collectors_model import RuleModel, UpdateRuleModel
...
async def create_rule(request: Request, 
    ...
    rule['services'] = []
    result = await request.app.mongodb["collectors"].insert_one(rule)
...
async def list_rules(request: Request):
    list_rules = []
    for doc in await request.app.mongodb["collectors"].find().to_list(length=1000):
       rule = {
           'id': str(doc['_id']),
           'name': doc['name'],
...
async def get_rule(id: str, 
                    request: Request):
    if (rule := await request.app.mongodb["collectors"].find_one({"_id": ObjectId(id) })) is not None:
...
async def update_rule(id: str, 
    ...
    col = request.app.mongodb["collectors"]
...
async def delete_rule(id: str, 
                      request: Request):
    delete_result = await request.app.mongodb["collectors"].delete_one(
        {"_id": ObjectId(id)})
...
async def copy_rule(id: str, request: Request):
    ...
    rule = await request.app.mongodb["collectors"].find_one({"_id": ObjectId(id)})
    if not rule:
        raise HTTPException(status_code=404, detail=f"Rule {id} not found")
    ...
    result = await request.app.mongodb["collectors"].insert_one(copy)
...
async def export_rules(request: Request):
    list_rules = []
    for doc in await request.app.mongodb["collectors"].find().to_list(length=10000):
       del doc['_id']
...
async def import_rules(request: Request):
    request.app.mongodb["collectors"].drop()
